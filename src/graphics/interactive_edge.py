"""
Arêtes interactives avec prise en charge de la déformation et contrôle via points d'ancrage.
Permet la manipulation avancée des connexions entre nœuds dans les diagrammes ManoDiag.
"""
...
class EdgeControlPoint(QGraphicsEllipseItem):
    """
    Représente un point de contrôle interactif pour la modification de la trajectoire d'une arête.
    Types : 'start', 'end' (ancrage sur nœud), 'control1', 'control2' (contrôle Bézier).
    """
    ...
    def __init__(self, edge, point_type: str):
        ...
        # Style visuel selon le type de point
    ...
    def hoverEnterEvent(self, event):
        """Effet visuel lors du survol du point de contrôle."""
        self.is_hovered = True
        self.setScale(1.2)
        super().hoverEnterEvent(event)

    def hoverLeaveEvent(self, event):
        """Réinitialise l'effet visuel après le survol."""
        self.is_hovered = False
        self.setScale(1.0)
        super().hoverLeaveEvent(event)

    def mousePressEvent(self, event):
        """Déclenche le déplacement du point de contrôle."""
    ...
    def mouseMoveEvent(self, event):
        """Gère le déplacement interactif du point de contrôle."""
...
class ClickablePathItem(QGraphicsPathItem):
    """
    Élément graphique personnalisé pour les arêtes, avec zone de clic élargie et gestion de la sélection.
    Facilite l'interaction utilisateur sur les courbes et lignes.
    """

    def __init__(self, parent_edge):
        super().__init__()
        self.parent_edge = parent_edge
        self.click_tolerance = 8
    ...
    def shape(self):
        """Retourne une forme élargie pour améliorer la sélection à la souris."""
    ...
    def hoverEnterEvent(self, event):
        """Effet visuel lors du survol de l'arête."""
    ...
    def hoverLeaveEvent(self, event):
        """Réinitialise le style après le survol."""
        if not self.parent_edge.is_selected:
    ...
    def mousePressEvent(self, event):
        """Gère la sélection et le mode Bézier via clic sur l'arête."""
        if event.button() == Qt.MouseButton.LeftButton:
    ...
    def itemChange(self, change, value):
        """Synchronise l'état de sélection avec l'arête parente."""
...
class InteractiveArrow(QGraphicsPolygonItem):
    """
    Flèche interactive pouvant être déplacée par l'utilisateur.
    Permet d'ajuster la direction et la position des têtes de flèche.
    """
    ...
    def mousePressEvent(self, event):
        """Déclenche le déplacement de la flèche."""
        if event.button() == Qt.MouseButton.LeftButton:
            self._dragging = True
            self.grabMouse()
            event.accept()
    ...
    def mouseMoveEvent(self, event):
        """Gère le déplacement interactif de la flèche."""
    ...
    def mouseReleaseEvent(self, event):
        """Termine le déplacement de la flèche et persiste l'état."""
...
class InteractiveEdge:
    """
    Gère la logique et l'affichage d'une arête interactive entre deux nœuds.
    Prend en charge la sélection, le mode Bézier, la persistance et les interactions graphiques.
    """

    _selected_edges = set()  # Gestion globale de la sélection
    ...
    def __init__(self, source_node, target_node, label: str = "", edge_type: str = "arrow"):
        ...
        # Points de contrôle Bézier et offsets personnalisés
        self.custom_start_offset = None
        self.custom_end_offset = None
    ...
    def deselect_all_edges(cls):
        """Désélectionne toutes les arêtes du diagramme."""
    ...
    def set_custom_start_point(self, scene_pos: QPointF):
        """Définit un point de départ personnalisé (offset relatif au nœud source)."""
    ...
    def set_custom_end_point(self, scene_pos: QPointF):
        """Définit un point d'arrivée personnalisé (offset relatif au nœud cible)."""
    ...
    def create_graphics_items(self, scene):
        """
        Instancie et ajoute tous les éléments graphiques de l'arête à la scène :
        - Path principal
        - Flèches
        - Label
        - Points de contrôle
        ...
        # Installation du wrapper de gestion de clic pour la désélection globale
        ...
        # Initialisation et application de l'état sauvegardé
        self.update_position()
        self.apply_saved_state()
        self.update_position()

    def create_control_points(self, scene):
        """Crée et ajoute les points de contrôle à la scène."""
    ...
    def toggle_selection(self):
        """Bascule l'état de sélection de l'arête."""
        if not self.is_selected:
            self.deselect_all_edges()
        self.set_selected(not self.is_selected)

    def set_selected(self, selected: bool):
        """Définit l'état de sélection et met à jour l'affichage."""
    ...
    def update_selection_visual(self):
        """Met à jour le style visuel selon l'état de sélection."""
        if self.is_selected:
            pen = QPen(QColor(231, 76, 60), self.line_width + 1)
            pen.setCapStyle(Qt.PenCapStyle.RoundCap)
            self.path_item.setPen(pen)
    ...
    def show_control_points(self, visible: bool):
        """Affiche ou masque les points de contrôle selon la sélection."""
        for point in self.control_points:
            point.setVisible(visible)

    def toggle_bezier_mode(self):
        """Active ou désactive le mode courbe Bézier pour l'arête."""
    ...
    def _persist_edge_state(self):
        """Sauvegarde l'état de l'arête (points, offsets, mode Bézier) via PositionManager."""
        pm = PositionManager()
    ...
    def apply_saved_state(self):
        """Recharge l'état persistant de l'arête depuis PositionManager."""
        pm = PositionManager()
        src = self.source_node.node_id
        dst = self.target_node.node_id
    ...
    def _compute_centers(self):
        """Calcule les centres des nœuds source et cible."""
    ...
    def _compute_endpoints(self):
        """Détermine les points de départ et d'arrivée de l'arête, en tenant compte des offsets personnalisés."""
    ...
    def initialize_bezier_points(self):
        """Initialise les points de contrôle Bézier selon la géométrie actuelle de l'arête."""
    ...
    def _update_label_position(self, start_pt: QPointF, end_pt: QPointF):
        """Positionne le label au centre de la ligne ou de la courbe Bézier."""
    ...
    def _arrow_polygon(self):
        """Retourne le triangle de la tête de flèche, orienté selon la direction de l'arête."""
        length = 14.0
        half_w = 6.0
    ...
    def _update_arrows(self, start_pt: QPointF, end_pt: QPointF):
        """Met à jour la position et l'orientation des flèches selon la géométrie de l'arête."""
    ...
    def _update_control_points_items(self, start_pt: QPointF, end_pt: QPointF):
        """Positionne les points de contrôle sur la scène selon la géométrie courante."""
    ...
    def update_position(self):
        """
        Recalcule la trajectoire de l'arête (ligne ou Bézier), met à jour la position des flèches,
        du label et des points de contrôle.
        """
    ...
    def remove_from_scene(self, scene):
        """
        Supprime tous les éléments graphiques associés à l'arête de la scène.
        Nettoie également les références dans les nœuds.
        """
        try:
            if self.source_node:
